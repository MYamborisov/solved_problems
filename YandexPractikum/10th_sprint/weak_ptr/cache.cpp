...
public:
    ...
    // Создаёт кэш, инициализированный объектом, играющим роль фабрики,
    ...
    std::shared_ptr<Value> GetValue(const Key& key) {
        ...
        if (!storage_.count(key) || storage_[key].expired()) {
            result = factory_(key);
            storage_[key] = result;
            return result;
        } else {
            return storage_[key].lock();
        }
    }
private:
...
public:
    ...
    std::shared_ptr<Book> operator()(const std::string& title) const {
        if (!store_.count(title)) {
            throw std::out_of_range("Book doesn't exist");
        }
        ++load_count_;
